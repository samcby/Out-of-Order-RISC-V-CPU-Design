...
// Simulation-only top-level integration testbench for top illegal smoke.
//
...
module tb_top_illegal_smoke;
    ...
    initial begin
        ...
        // Main program:
        //   mtvec = 0x7f0; execute a non-zero illegal instruction; after
        //   mret, set a0 = 0x66.
        ...
        //   advance mepc past the illegal instruction, copy mcause to a1,
        ...
        check_ok(dut.u_dispatch.u_rob.empty == 1'b1, "ROB drained after illegal instruction smoke program");
        check_ok(a0_value == 32'h00000066, "Illegal smoke returned from handler and executed post-trap instruction");
        check_ok(a1_value == 32'h00000002, "Illegal smoke a1(x11) captured illegal-instruction mcause 2");
        check_ok(a2_value == 32'hffffffff, "Illegal smoke a2(x12) captured illegal instruction in mtval");
        check_ok(mtvec_value == 32'h000007f0, "Illegal smoke mtvec retains handler address 0x7f0");
        check_ok(mepc_value == 32'h0000000c, "Illegal smoke handler advanced mepc to 0x0c");
        check_ok(mcause_value == 32'h00000002, "Illegal smoke mcause records illegal instruction");
        check_ok(mtval_value == 32'hffffffff, "Illegal smoke mtval records faulting instruction");
        check_ok(mstatus_value == 32'h00000080, "Illegal smoke mret set MPIE and restored disabled MIE");

        if (fail_count == 0) begin
            $display("==== tb_top_illegal_smoke PASS ====");
        end else begin
            $display("==== tb_top_illegal_smoke FAIL (%0d errors) ====", fail_count);
        end
