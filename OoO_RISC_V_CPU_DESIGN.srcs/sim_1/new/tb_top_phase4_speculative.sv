...
// Simulation-only top-level integration testbench for top phase4 speculative.
//
...
module tb_top_phase4_speculative;
    ...
    initial begin
        ...
        // 0x08: addi x6, x0, 99     wrong-path, may speculate
        // 0x0C: addi x7, x0, 77     wrong-path, may speculate
        ...
        $display("[SUMMARY] issue_count=%0d wrong_path_issue_count=%0d commit_count=%0d wrong_path_commit_count=%0d branch_pending=%0b rob_head_valid=%0b rob_head_complete=%0b rob_head_rd=%0d",
                 issue_count, wrong_path_issue_count, commit_count, wrong_path_commit_count,
                 dut.branch_pending_q, rob_head_valid, rob_head_complete, rob_head_rd);

        check_ok(issue_count >= 4, "saw useful issue traffic");
        check_ok(commit_count >= 4, "saw useful commit traffic");
        check_ok(wrong_path_commit_count == 0, "wrong-path instructions never committed");

        if (fail_count == 0) begin
            $display("==== tb_top_phase4_speculative PASS ====");
        end else begin
            $display("==== tb_top_phase4_speculative FAIL (%0d errors) ====", fail_count);
        end
