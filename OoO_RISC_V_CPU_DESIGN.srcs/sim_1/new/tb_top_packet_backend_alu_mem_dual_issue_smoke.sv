...
module tb_top_packet_backend_alu_mem_dual_issue_smoke;
    ...
    int fail_count;
    int alu_mem_dual_issue_count;
    int lane1_alu_wb_count;
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            alu_mem_dual_issue_count <= 0;
            lane1_alu_wb_count <= 0;
            dual_commit_count <= 0;
        end else begin
            #1;
            if (dut.issue_if.valid && dut.issue_if.ready &&
                dut.issue1_if.valid && dut.issue1_if.ready &&
                (dut.issue_if.data.fu_sel == FU_MEM) &&
                (dut.issue_if.data.datapath.pc == 32'd0) &&
                (dut.issue1_if.data.fu_sel == FU_ALU) &&
                (dut.issue1_if.data.datapath.pc == 32'd4)) begin
                alu_mem_dual_issue_count <= alu_mem_dual_issue_count + 1;
            end

            if (dut.wb1_valid && (dut.wb1_result == 32'd9)) begin
                lane1_alu_wb_count <= lane1_alu_wb_count + 1;
            end
    ...
    initial begin
        ...
        write_word(32'd0,  32'h00002283); // lw   x5,0(x0), lane0 MEM
        write_word(32'd4,  32'h00900593); // addi x11,x0,9, lane1 ALU
        write_word(32'd8,  32'h00028513); // addi x10,x5,0, waits for load
        ...
        $display("[SUMMARY] alu_mem_dual_issue=%0d lane1_alu_wb=%0d dual_commit=%0d x5=0x%08h a0=0x%08h a1=0x%08h rob_empty=%0b",
                 alu_mem_dual_issue_count,
                 lane1_alu_wb_count,
                 dual_commit_count,
                 x5_value,
                 a0_value,
        ...
        check_ok(alu_mem_dual_issue_count >= 1,
                 "packet backend issued MEM on lane0 with ALU on lane1");
        check_ok(lane1_alu_wb_count >= 1,
                 "lane1 ALU wrote back while lane0 load was in flight");
        check_ok(dual_commit_count >= 1,
                 "oldest load and younger ALU committed together after load completion");
        check_ok(dut.u_dispatch_packet.u_rob_2w.empty == 1'b1,
                 "packet backend ROB drained after ALU+MEM dual issue smoke program");
        check_ok(x5_value == 32'd8,
                 "load result wrote x5=8");
        check_ok(a0_value == 32'd8,
                 "dependent consumer read loaded x5 into a0");
        check_ok(a1_value == 32'd9,
                 "lane1 ALU produced a1=9");

        if (fail_count == 0) begin
            $display("==== tb_top_packet_backend_alu_mem_dual_issue_smoke PASS ====");
        end else begin
            $display("==== tb_top_packet_backend_alu_mem_dual_issue_smoke FAIL (%0d errors) ====", fail_count);
        end
