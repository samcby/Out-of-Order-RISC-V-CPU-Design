...
// Simulation-only top-level integration testbench for top load misaligned smoke.
//
...
module tb_top_load_misaligned_smoke;
    ...
    initial begin
        ...
        // Main program:
        //   mtvec = 0x7f0; perform misaligned lw from address 2; after mret,
        //   set a0 = 0x77.
        write_word(32'd0,  32'h7f000093); // addi  x1,x0,0x7f0
        write_word(32'd4,  32'h30509073); // csrrw x0,mtvec,x1
        write_word(32'd8,  32'h00202283); // lw    x5,2(x0)
        write_word(32'd12, 32'h07700513); // addi  x10,x0,0x77

        // Trap handler at 0x7f0:
        //   advance mepc past the faulting load, copy mcause to a1, copy
        //   mtval to a2, then return.
        ...
        check_ok(dut.u_dispatch.u_rob.empty == 1'b1, "ROB drained after load misaligned smoke program");
        check_ok(a0_value == 32'h00000077, "Load misaligned smoke returned from handler and executed post-trap instruction");
        check_ok(a1_value == 32'h00000004, "Load misaligned smoke a1(x11) captured load-address-misaligned mcause 4");
        check_ok(a2_value == 32'h00000002, "Load misaligned smoke a2(x12) captured fault address in mtval");
        check_ok(mtvec_value == 32'h000007f0, "Load misaligned smoke mtvec retains handler address 0x7f0");
        check_ok(mepc_value == 32'h0000000c, "Load misaligned smoke handler advanced mepc to 0x0c");
        check_ok(mcause_value == 32'h00000004, "Load misaligned smoke mcause records load address misaligned");
        check_ok(mtval_value == 32'h00000002, "Load misaligned smoke mtval records faulting address");
        check_ok(mstatus_value == 32'h00000080, "Load misaligned smoke mret set MPIE and restored disabled MIE");

        if (fail_count == 0) begin
            $display("==== tb_top_load_misaligned_smoke PASS ====");
        end else begin
            $display("==== tb_top_load_misaligned_smoke FAIL (%0d errors) ====", fail_count);
        end
