...
// Simulation-only integration-level packet-backend testbench for top packet backend lane1 squash smoke.
//
...
module tb_top_packet_backend_lane1_squash_smoke;
    ...
    int fail_count;
    int lane1_wrong_issue_count;
    int lane1_wrong_wb_count;
    int redirect_count;
    preg_t a0_preg;
    preg_t a1_preg;
    preg_t x12_preg;
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            lane1_wrong_issue_count = 0;
            lane1_wrong_wb_count = 0;
            redirect_count = 0;
        end else begin
            #1;
            ...
            if (dut.wb1_valid && (dut.wb1_result == 32'h00000011)) begin
                lane1_wrong_wb_count = lane1_wrong_wb_count + 1;
            end
    ...
    initial begin
        ...
        $display("[SUMMARY] lane1_wrong_issue=%0d lane1_wrong_wb=%0d redirects=%0d a0_preg=%0d a0=0x%08h a1_preg=%0d a1=0x%08h x12_preg=%0d x12=0x%08h rob_empty=%0b",
                 lane1_wrong_issue_count,
                 lane1_wrong_wb_count,
                 redirect_count,
                 a0_preg, a0_value,
                 a1_preg, a1_value,
                 x12_preg, x12_value,
        ...
        check_ok(lane1_wrong_issue_count >= 1,
                 "wrong-path ALU issued on lane1 beside resolving branch");
        check_ok(lane1_wrong_wb_count >= 1,
                 "wrong-path lane1 ALU wrote a speculative physical register");
        check_ok(redirect_count >= 1,
                 "taken branch redirected fetch");
        check_ok(dut.u_dispatch_packet.u_rob_2w.empty == 1'b1,
                 "packet backend ROB drained after lane1 squash smoke program");
        check_ok(a0_value == 32'h00000055,
                 "target path a0(x10) overrides squashed lane1 wrong-path value");
        check_ok(a1_value == 32'h00000066,
                 "target path a1(x11) completed");
        check_ok(x12_value == 32'h00000000,
                 "wrong-path x12 write was squashed");

        if (fail_count == 0) begin
            $display("==== tb_top_packet_backend_lane1_squash_smoke PASS ====");
        end else begin
            $display("==== tb_top_packet_backend_lane1_squash_smoke FAIL (%0d errors) ====", fail_count);
        end
