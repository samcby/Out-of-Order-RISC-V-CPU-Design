    ...
    always_comb begin
        ...
        if (control_signal.mem_read) begin
            unique case (control_signal.funct3)
                ...
                3'b101: load_result = {16'h0, load_half};              // LHU
    ...
    always_comb begin
        ...
        if (control_signal.mem_write) begin
            unique case (control_signal.funct3)
                3'b000: begin                            // SB
                    unique case (byte_off)
                        2'd0: next_word[7:0]   = datapath.src2_value[7:0];
                        2'd1: next_word[15:8]  = datapath.src2_value[7:0];
                        2'd2: next_word[23:16] = datapath.src2_value[7:0];
                        default: next_word[31:24] = datapath.src2_value[7:0];
                    endcase
                end
                3'b001: begin                            // SH
                    unique case (byte_off)
